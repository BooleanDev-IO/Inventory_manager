...
class DateTimeField extends FormField<DateTime> {
  ...
  static DateTime combine(DateTime date, TimeOfDay? time) => DateTime(date.year, date.month, date.day, time?.hour ?? 0, time?.minute ?? 0);
...
class _DateTimeFieldState extends FormFieldState<DateTime> {
  ...
  bool shouldShowClearIcon([InputDecoration? decoration]) => widget.resetIcon != null && (hasText || hasFocus) && decoration?.suffixIcon == null;
}
